...
with tab_gtn:
    ...
    # Add structural and tactical GTN percentages
    df_structural = df_filtered.groupby("client_name")["structural_gtn_pct"].mean().reset_index()
    df_tactical = df_filtered.groupby("client_name")["tactical_gtn_pct"].mean().reset_index()
    
    df_gtn_summary = df_gtn_summary.merge(df_structural, on="client_name", how="left")
    df_gtn_summary = df_gtn_summary.merge(df_tactical, on="client_name", how="left")
    df_gtn_summary.columns = ["client_name", "units_sold", "gross_sales", "gtn_amount", "nts", "gtn_pct", "structural_gtn_pct", "tactical_gtn_pct"]
...
with tab_gp_drivers:
    ...
    st.caption("Overall GP drivers for the selected filters")
    
    # Create a summary of drivers
    total_gp_drivers = df_filtered["gp_std"].sum()
    total_gts_drivers = df_filtered["gross_sales"].sum()
    total_cogs_drivers = df_filtered["cogs"].sum()
    
    # Calculate the impact of each driver
    volume_impact = total_units * (avg_price - avg_cogs)
    price_impact_total = total_units * avg_price
    cost_impact_total = total_units * avg_cogs
    
    summary_data = [
        ("Total GP", f"£{total_gp_drivers:,.0f}", f"{gp_pct:.1f}% of Price"),
        ("Total GTS", f"£{total_gts_drivers:,.0f}", ""),
        ("Total COGS", f"£{total_cogs_drivers:,.0f}", f"{(total_cogs_drivers / total_gts_drivers * 100):.1f}% of GTS"),
        ("Volume Impact", f"{total_units:,.0f} units", f"Avg GP: £{avg_gp:.2f}/unit"),
        ("Price Impact", f"£{price_impact_total:,.0f}", f"Avg Price: £{avg_price:.2f}/unit"),
        ("Cost Impact", f"£{cost_impact_total:,.0f}", f"Avg COGS: £{avg_cogs:.2f}/unit")
    ]
    ...
    st.dataframe(
        df_summary.style.applymap(
            lambda x: "font-weight: 600;" if "GP" in str(x) or "Total" in str(x) else "",
            subset=["Driver"]
        ),
        use_container_width=True,
        hide_index=True
